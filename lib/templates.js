...
module.exports = {
	...
	generateElements2: function(model, elementTemplates, dest) {
		...
		return new Promise(function(success, error) {
			...
			for (var i = 0; i < model.children.length; ++i) {
				...
				if (typeof elementTemplates[child.elementType] != 'object') {
				...
				promises.push(that.generateElement(child, elementTemplates[child.elementType], dest));
			}
			Promise.all(promises).then(success, error);
		});
	},
	...
	processTemplate: function(filename, template, model) {
		if (!template) return new Promise(function(s,e) { e("No template content for " + filename); });
				...
				while (ctx.pos < ctx.src.length) {
					...
					if (tag.charAt(0) == '$') {
						...
						if (!ctx.ignore) {
							...
							if (expr.startsWith("?")) {
								try {
									val = that.evaluate(expr.substring(1), ctx.model);
								} catch (e) {
									val = "";
								}
							} else
								val = that.evaluate(tag, ctx.model);
