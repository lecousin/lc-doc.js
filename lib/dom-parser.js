...
DOMElement.prototype.toString = function() {
	...
	var inside = this.getInnerXML();
	if (inside.length > 0)
		xml += ">" + inside + "</" + this.nodeName + ">";
	else
		xml += "/>";
	return xml;
};
...
module.exports = {
	...
	parse: function(xml) {
		return new Promise(function(success, error) {
			...
			parser.on('finish', () => {
			    // Stream is completed
				//console.log(require('util').inspect(dom, false, null));
				success(dom);
			});
			
			// override function to keep spaces in text nodes (remove trim())
			parser._onStartNewTag = function() {
