...
var searchNode = function(node, pos) {
	if (node.start <= pos && node.end >= pos) {
		...
		for (var name in node) {
			...
			for (var i = 0; i < children.length; ++i) {
				...
				var n = searchNode(children[i], pos);
				if (n) return n;
			}
		}
	} else if (node.start > pos)
		return node;
	return null;
};
...
var createClass = function(root, text) {
	var i = text.indexOf(' ');
	var name, description;
	if (i < 0) {
		name = text.trim();
...
var processMethodComment = function(c, m, node) {
	...
	//console.log("Node: " + require('util').inspect(node, { depth: 2 } ));
}

var processComment = function(comment, ast, root, context) {
	//console.log("Comment: " + comment.text);
	var c = parseComment(comment.text);
	//console.log("Parsed: " + require('util').inspect(c, { depth: 2 } ));
	var t = getTag(c, "namespace");
	if (t !== null) {
		createNamespace(root, t);
		return null;
	}
	t = getTag(c, "class");
	if (t !== null) {
		return createClass(root, t);
	}
	...
	var node = searchNode(ast, comment.pos);
	if (node) {
		if (context && context.type == 'class') {
